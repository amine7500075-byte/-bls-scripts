...
(function () {
  ...
  class OTPManager {
    ...
    async loginMailTm() {
      try {
        const res = await this._request("POST", "https://api.mail.tm/token", { address: config.otpServer.email, password: config.otpServer.password });
    ...
    async fetchOTP() {
      ...
      try {
        ...
        const messages = (res?.["hydra:member"] || []).filter((msg) => (msg.from?.name?.includes("BLS") || msg.from?.address?.includes("bls")) && msg.to?.some((r) => r.address === email));
    ...
    async _request(method, url, data, useAuth) {
      return new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
          method, url,
          headers: Object.assign({ "Content-Type": "application/json" }, useAuth && this.token ? { Authorization: `Bearer ${this.token}` } : {}),
    ...
    loadClientsFromSheet().then((clients) => {
      ...
      if (saved) {
        try {
          ...
          document.querySelectorAll(".client-btn").forEach((b) => { if (b.innerHTML.includes(parsed.name)) { b.style.background = "linear-gradient(90deg,#28a745,#20c997)"; b.classList.add("selected"); } });
        } catch {}
      }
    });
  }
  ...
  function checkForUpdates() {
    ...
    GM_xmlhttpRequest({
      method: "GET",
      url: "https://raw.githubusercontent.com/amine7500075-byte/bls-scripts/main/MADRID-BOT.user.js",
      onload(r) {
        try {
          ...
          if (!newer) {
            ...
            n.style.cssText = "position:fixed;top:60px;right:10px;background:linear-gradient(135deg,#3D0C02,#5C1A08);color:#F5E6D0;padding:12px;border-radius:8px;z-index:10000;max-width:300px;border-left:5px solid #C9853A;font-family:Arial,sans-serif;";
            n.innerHTML = `<div style="font-weight:bold;display:flex;align-items:center;gap:8px;"><span></span> Script à jour (v${currentVer})</div><div style="font-size:12px;margin-top:4px;color:#C4A882;">Vous utilisez la dernière version.</div>`;
            document.body.appendChild(n);
            setTimeout(() => { n.style.opacity = "0"; n.style.transition = "opacity 0.5s"; setTimeout(() => n.remove(), 500); }, 5000);
            return;
          }
          const n = document.createElement("div");
          ...
          n.innerHTML = `<div style="font-weight:bold;margin-bottom:8px;color:#E8A84E;">🔄 Mise à jour disponible (v${m[1]})</div><div style="font-size:13px;margin-bottom:12px;color:#C4A882;">Une nouvelle version est disponible.</div><button id="upd-now-btn" style="background:linear-gradient(135deg,#C9853A,#E8A84E);color:#3D0C02;border:none;padding:8px 12px;border-radius:6px;cursor:pointer;font-size:12px;font-weight:bold;">Mettre à jour maintenant</button><button id="upd-later-btn" style="background:transparent;color:#C4A882;border:none;padding:8px 12px;cursor:pointer;font-size:11px;">Rappeler plus tard</button>`;
          document.body.appendChild(n);
          document.getElementById("upd-now-btn").onclick = () => { window.open("https://raw.githubusercontent.com/amine7500075-byte/bls-scripts/main/MADRID-BOT.user.js"); n.remove(); };
          ...
          setTimeout(() => { if (n.isConnected) { n.style.opacity = "0"; n.style.transition = "opacity 0.5s"; setTimeout(() => n.remove(), 500); } }, 30000);
        } catch {}
      },
    });
  }
  ...
  class DataProtectionHandler {
    ...
    async findAcceptanceLink(email) {
      try {
        ...
        const messages = (res?.["hydra:member"] || []).filter((msg) => (msg.from?.name?.includes("BLS") || msg.from?.address?.includes("bls") || msg.subject?.includes("Data Protection")) && msg.to?.some((r) => r.address === email));
    ...
    handleConsentAndGenerate() {
      ...
      const iv = setInterval(() => {
        try {
          ...
          if (gen && gen.style.display !== "none" && gen.offsetParent !== null) { gen.click(); clearInterval(iv); return; }
        } catch {}
